...
let designerGrid = document.getElementsByClassName('designer-grid')[0];
for (let d in designers)
  designerGrid.innerHTML += `
    <div class='designer' data-target='${d}'>
...
let designerModal = document.getElementById('designer-modal');
let designerModalName = document.getElementById('designer-modal-name');
let designerModalContent = document.getElementById('designer-modal-content');
let designerModalPortrait = document.getElementById('designer-modal-portrait');
let modalBackdrop = document.getElementsByClassName('modal-backdrop')[0];
let designerPortraits = document.getElementsByClassName('designer');
for (let d of designerPortraits)
  d.addEventListener('click', (e) => {
    let target = e.target.dataset.target;
    designerModalName.innerText = target;
    designerModalContent = designers[target].desc;
    activateDesignerModal();
  });

modalBackdrop.addEventListener('click', closeDesignerModal);

function activateDesignerModal() {
  designerModal.style.display = 'block';
  setTimeout(() => {
    designerModal.className = 'designer-modal active';
  }, 10);
  document.body.style.overflow = 'hidden';
}

function closeDesignerModal() {
  designerModal.className = 'designer-modal';
  setTimeout(() => {
